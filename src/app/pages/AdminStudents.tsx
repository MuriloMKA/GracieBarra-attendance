...
export const AdminStudents: React.FC = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "active" | "inactive" | "all"
  >("active");
  const [programFilter, setProgramFilter] = useState<Program | "all" | "GBK">(
    "all",
  );
  ...
  const getStudentDisplayProgram = (student: Student): Program =>
    calculateProgram(
      student.program,
      student.belt,
      student.degrees,
  ...
  const programFilterOptions: Array<{
    value: Program | "all" | "GBK";
    label: string;
  }> = [
  ...
  const visibleStudents = students.filter((s) => {
    ...
    const displayProgram = getStudentDisplayProgram(s);
    if (programFilter !== "all") {
      if (programFilter === "GBK") {
        if (!displayProgram.startsWith("GBK")) return false;
      } else if (displayProgram !== programFilter) {
        return false;
      }
    }
    ...
    return (
      ...
      normalizeString(displayProgram).includes(q)
    );
  });
  const pageSize = 10;
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {programFilterOptions.map((filter) => (
            <button
              key={filter.value}
              type="button"
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100">
                {currentPageStudents.map((student) => {
                  ...
                  const displayProgram = getStudentDisplayProgram(student);
                  return (
                    <tr
                      key={student.id}
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <span
                          className={`px-2.5 py-1 rounded-lg text-xs font-bold ${displayProgram.startsWith("GBK") ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700"}`}
                        >
                          {getProgramLabel(displayProgram)}
                        </span>
                      </td>
                      <td className="px-5 py-4">
                        <span
                  ...
                  {degreeProgress.weeksRequired !== null && (
                    <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-xl">
                      ...
                      <div className="flex justify-between text-xs text-gray-600">
                        <span>
                          {progressValue} / {progressRequiredValue}{" "}
                          {progressUnit}
                        </span>
          ...
          >
            ...
            >
              ...
              <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-xl text-xs text-blue-900">
                <strong>Regras de Programa:</strong> GB1 = Faixa branca 1-2
                graus • GB2 = Faixa branca 3-4 graus • GB3 = Faixa azul em
                diante • GBK
              </div>
