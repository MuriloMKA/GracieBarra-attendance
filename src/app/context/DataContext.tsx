...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const checkIn = async (
    ...
    try {
      ...
      setAttendance([
        ...attendance,
        { ...newAttendance, id: newAttendance._id || newAttendance.id },
      ]);
  ...
  const confirmAttendance = async (attendanceId: string) => {
    try {
      ...
      setAttendance(
        attendance.map((a) =>
          a.id === attendanceId || a._id === attendanceId
            ? { ...a, confirmed: true }
            : a,
        ),
      );
  ...
  const updateSpecialDates = async (
    studentId: string,
    specialDates: SpecialDate[],
  ) => {
    try {
  ...
  const updateClass = async (cls: JJClass) => {
    try {
      const classId = (cls.id || cls._id) as string;
      const updated = await classService.update(classId, cls);
      setClasses(
        classes.map((c) =>
          (c.id || c._id) === classId
            ? { ...updated, id: updated._id || updated.id }
  ...
  const addClass = async (cls: Omit<JJClass, "id">) => {
    try {
      const created = await classService.create(cls);
      setClasses([...classes, { ...created, id: created._id || created.id }]);
  ...
  const deleteClass = async (classId: string) => {
    try {
      ...
      setClasses(classes.filter((c) => (c.id || c._id) !== classId));
